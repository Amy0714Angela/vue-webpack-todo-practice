...
if(isDev){
    config.module.rules.push(
        {
            ...
            use: [
                ...
                'css-loader',                       //css-loader处理css
                {
                    ...
                    options: {
                        sourceMap: true,            //sass-loader和postcss-loader自己都会生成sourceMap,如果前面sass-loader已生成了sourceMap
                    }                               //那么postcss-loader可以直接引用前面的sourceMap
                },
                'sass-loader'                     //处理scss的css预处理器的问题件,转换成css后,抛给上一层的css-loader
            ]
        }
    )                                             //如果是测试环境下的一些配置
    ...
    config.module.rules.push(
        {
            ...
            use: ExtractPlugin.extract({
                ...
                use: [
                    ...
                    'sass-loader'                     //处理sass的css预处理器的问题件,转换成css后,抛给上一层的css-loader
                ]
            })
        },
    )/*,
    config.plugins.push(
        ...
        new webpack.optimization.splitChunks({          //定义静态文件打包
            name: 'vendor'
        }),
        new webpack.optimization.splitChunks({         //将app.js文件中一些关于webpack文件的配置单独打包出为一个文件,用于解决部分浏览器长缓存问题   
            name: 'runtime'
        })
    )*/
} 
